  ...
  <el-dialog :title="`选择商品${chooseSingle ?'（单选）' : ''}`" :visible="value" class="v-dialog v-max-height-body" append-to-body width="1000px" top="10vh" @close="beforeClose">
    <el-form :model="formData" :inline="true" label-width="70px" class="searchForm" @submit.native.prevent>
      ...
      <!--<el-form-item label="店铺">-->
        <!--<el-select size="small" v-model="formData.shopIds" clearable  placeholder="请选择店铺" filterable>-->
      ...
      <el-form-item label="商品货号">
        <el-input size="small" v-model=" formData.goodsSn" :maxlength="50"  @keyup.enter.native="toSearch"> </el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 判断是否选中
    isCheck (row) {
      if (this.choosePageIdArr.has(row.goodsId)) {
        this.choosePageIdArr.set(row.goodsId, row)
      }
      return this.choosePageIdArr.has(row.goodsId)
    },
    // 点击选中
    toCheck (item, isChecked) {
      if (!isChecked) { // 取消选中
        this.choosePageIdArr.delete(item.goodsId)
      ...
      this.choosePageIdArr.set(item.goodsId, item)
      this.listData = JSON.parse(JSON.stringify(this.listData))
    },
    // 关闭弹窗
    beforeClose () {
    ...
    toClear () {
      ...
      this.formData = { // 请求参数
        catId: '',
        catIdArr: [],
        shopIds: this.shopIds && this.shopIds.length ? this.shopIds.join(',') : '',
        goodsName: '',
        goodsSn: '',
        productSku: '',
        currentPage: 1,
  ...
  watch: {
    value (newValue, oldValue) {
      ...
      this.toClear()
      if (this.chooseId && this.chooseId !== '') { // 显示的时候
        let chooseArr = this.chooseId.split(',')
        this.choosePageIdArr = new Map()
        chooseArr.map(data => {
          this.choosePageIdArr.set(Number(data), {goodsId: +data})
        })
      }
    }
  }
