...
<template>
  <div class="imageEditWrap">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>编辑导航模块</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="v-wrap listWrap"  v-loading="mainLoading">
      <div>
        <h3 class="v-title">编辑导航模块</h3>
      </div>
      <el-form :model="formData" :rules="formRules" ref="formDataRef" label-width="80px" class="v-form detailForm" @submit.native.prevent>
        ...
        <el-form-item label="样式" prop="style">
          <div class="styleItem">
            <el-radio v-model="formData.style" :label="3">图文导航</el-radio>
      ...
      <div class="detailTable">
        <!-- 图片设置模块 -->
        <imgSetModule :listData="listData" showIconText desc="温馨提示：最多加入8个导航图标，仅支持 gif、 jpeg、 png、 bmp 4种格式"/>
        <!-- 上传图片组件 -->
        <singleUpload :listData="listData" :limit="8" :size="3" title="添加导航图标" desc="建议上传尺寸90*90，3MB以内"/>
      ...
      if (parseInt(res.code) === 1) {
        ...
        if (res.data.pageModuleContent) {
          ...
          if (res.data.pageModuleContent.dataConfig) {
            this.formData.style = res.data.pageModuleContent.dataConfig.style || 3
    ...
    toSaveData () {
      this.$refs.formDataRef.validate(async valid => {
        ...
        let req = {
          ...
          falsePageModuleContentId: this.falsePageModuleContentId,
          pagesToken: this.pagesToken,
          moduleType: 3, // 0:首页  1:图片编辑 2:商品编辑 3:导航编辑 4:拼团编辑 5:秒杀编辑
