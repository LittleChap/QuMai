...
export default {
  ...
  methods: {
    async getLogo () {
      let res = await this.$post('login_logo.do')
      if (+res.code === 1) {
        this.logo = res.data.backend_login_logo
      } else {
    ...
    async getVerifyCode () {
      ...
      let res = await this.$post('mobileCommon/getValidateCode.do', {validToken: this.loginForm.validToken})
    ...
    handleLogin () {
      this.$refs.loginForm.validate(async valid => {
        if (valid) {
          ...
          let res = await this.$post('admin_login.do', reqData)
          this.loading = false
          if (parseInt(res.code) === 1) {
            this.$store.dispatch('SetTokenInfo', res.data.user)
