<template>
  <div class="v-wrap detailWrap">
    ...
    <el-form class="formBox" ref="ruleForm" :rules="formRules" :model="formData" label-width="110px" v-loading="mainLoading">
      <el-form-item>
        <el-button type="danger" class="v-button" icon="el-icon-delete" v-if="roleId > -1" @click="deleteRole" :loading="deleteLoading" >删除</el-button>
      </el-form-item>
      ...
      <el-form-item label="权限设置" class="inputItem">
        <el-tree
          ...
          <span class="custom-tree-node" slot-scope="{ node, data }">
            ...
            <!--<div  v-if="!data.children">-->  <!-- 做第四级菜单时用 -->
              <!--|&#45;&#45;-->
              <!--<el-checkbox v-model="checked">新建</el-checkbox>-->
...
export default {
  ...
  methods: {
    ...
    async getDetail () {
      ...
      let res = await this.$post('role/getRoleDetail.do', {roleId: this.roleId})
      this.mainLoading = false
      if (parseInt(res.code) === 1) {
        this.formData = res.data.role
        this.formData.name = res.data.role.roleName
        if (res.data.role.rightsStr) {
          this.defaultChecked = res.data.role.rightsStr.split(';')
          this.$refs.tree.setCheckedKeys(this.defaultChecked)
        }
        if (res.data.role.roleName === '系统管理员') {
          this.$message.error('该账号不支持编辑')
          setTimeout(() => {
            this.$router.replace('/system/permission/permissionList')
          }, 1500)
        }
      } else {
    ...
    deleteRole () {
      ...
      this.$confirm('是否确认删除？', '确认', {
        ...
        let res = await this.$post('role/deletedRole.do', {roleId: this.roleId})
